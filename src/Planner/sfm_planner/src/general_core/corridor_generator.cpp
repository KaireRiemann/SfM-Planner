    ...
    CorridorGenerator::SearchPolytopeOnPath(const vec_Vec3f &path, PolytopeVec &sfcs,
        ...
        while (cnt_loop++ < max_loop) {
            ...
            if (!GeneratePolytopeFromLine(seed_lines.back(), temp_poly)) {
                cout << YELLOW << " -- [GeneralPlanner] GeneratePolytopeFromLine failed." << RESET << endl;
                return false;
            }
            ...
            if (!sfcs.empty()) {
                ...
                if (interior_depth < min_overlap_threshold_) {
                    if (!GeneratePolytopeFromPoint(path[first_id], temp_poly_fix_p)) {
                        cout << YELLOW << " -- [GeneralPlanner] GeneratePolytopeFromPoint failed." << RESET << endl;
                        return false;
                    }
                    ...
                    if (interior_depth <= 0.01) {
                        ros_ptr_->warn(
                                " -- [GeneralPlanner] Cannot find continuous corridor on path, overlap only {}, force return.",
                                interior_depth);
// viz for debug
...
//                        exit(-1);
        ...
        if (cnt_loop >= max_loop) {
            cout << YELLOW << " -- [GeneralPlanner] Reach max iteration, failed." << RESET << endl;
            return false;
        }
...
//        box_max.z() = std::min(box_max.z(), virtual_ceil_height_);
    ...
    bool CorridorGenerator::GeneratePolytopeFromPoint(const Vec3f &pt, Polytope &polytope) {
        ...
        if (success == SUCCESS) {
            ...
            cout << YELLOW << " -- [GeneralPlanner] CSpaceFiri failed." << RESET << endl;
