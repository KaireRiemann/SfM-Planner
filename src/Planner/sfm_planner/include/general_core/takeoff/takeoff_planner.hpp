...
public:
    explicit TakeoffPlanner(PlannerContext context)
        : context_(std::move(context)) {}

    PlanResult plan(const TakeoffPlanRequest &request,
                    TaskPlanContext context = {}) const {
        if (!context_.valid()) {
            return makeResult(request.plan_request, general_utils::INIT_ERROR, std::move(context));
        }
        const int ret = context_.planner().PlanDynamicTakeoffFromRest(request.surface,
                                                                      request.new_task);
        return makeResult(request.plan_request, ret, std::move(context));
    }

    PlanResult replan(const TakeoffPlanRequest &request,
                      TaskPlanContext context = {}) const {
        if (!context_.valid()) {
            return makeResult(request.plan_request, general_utils::INIT_ERROR, std::move(context));
        }
        const int ret = context_.planner().ReplanDynamicTakeoffOnce(request.surface,
                                                                    request.new_task);
        return makeResult(request.plan_request, ret, std::move(context));
    }
